...
GHE_CONSOLE_PASSWD=`cat $ADMININFO| grep github_console_password | awk -F: '{print $2}'`
...
curl -i -k -L -F license=@/tmp/${GHE_LICENSE} -F password=${GHE_CONSOLE_PASSWD} -X POST https://${EC2_IP}:8443/setup/api/start 

# Initiate the configuration process
curl -i -k -L -X POST https://api_key:${GHE_CONSOLE_PASSWD}@localhost:8443/setup/api/configure

# Get the default settings
curl -i -k -L https://api_key:${GHE_CONSOLE_PASSWD}@localhost:8443/setup/api/settings

# Check the configuration status and continue to check until the configuration is complete
CONFIG_STATUS=`curl -k -L https://api_key:${GHE_CONSOLE_PASSWD}@localhost:8443/setup/api/configcheck | awk -F, '{print $NF}' | awk -F: '{print $NF}' |tail -n1 `
